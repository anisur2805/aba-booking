...
class Teachers_DT extends \WP_List_Table {
      private $_items;
      private $users_data;
      ...
      private function get_users_data( $search = '' ) {
            ...
            if( !empty( $search ) ) {
                  return $wpdb->get_results(
                        "SELECT id, name, email, teacher_id, department from {$wpdb->prefix}aba_booking_teacher WHERE id LIKE '%{$search}%' OR name Like '%{$search}%' OR email Like '%{$search}%'", ARRAY_A );
            } else {
                  return $wpdb->get_results(
                        "SELECT id, name, email, teacher_id, department from {$wpdb->prefix}aba_booking_teacher", ARRAY_A );
            }
      }
      ...
      public function get_columns() {
            $columns = array(
                  ...
                  'name'            => __('Name', 'dbdemo'),
                  'teacher_id'      => __('Student ID', 'dbdemo'),
      ...
      public function column_name($item) {
            $nonce = wp_create_nonce('teacher-edit');
            $actions = [];
      ...
      public function get_sortable_columns() {
            return array(
                  'id'              => array('id', false),
                  'teacher_id'      => array('teacher_id', false),
      ...
      private function table_data($data) {
            global $wpdb;
            $data = $wpdb->get_results($wpdb->prepare("SELECT name FROM {$wpdb->prefix}aba_booking_teacher"), ARRAY_A);
